...
import BlockMultiplay from '../Scripts/Multiplay/BlockMultiplay';
import DebugScreen from '../Scripts/DebugScreen'
...
export default class ClientStarter extends ZepetoScriptBehaviour {
    ...
    private OnStateChange(state: State, isFirst: boolean) {
        ...
        if (isFirst) {
            ...
            ZepetoPlayers.instance.OnAddedPlayer.AddListener((sessionId: string) => {
                ...
                // 캐릭터가 블록 위에 먼저 올라와있었다면 관련 처리 진행
                BlockMultiplay.GetInstance().CheckPlayerOnBlock(sessionId);
            });
        }
    ...
    private OnUpdatePlayer(sessionId: string, player: Player) {
        ...
        // 플레이어가 블록 위에 있거나 블록 사이를 건너는 중일 경우에는 위치를 동기화하지 않습니다.
        if (player.isOnBlock) {
            return;
        }
